...
# from nems.utilities.io import load_matlab_matrix  #TODO: Where is this?
...
def load_site_raster(batch, site, options, runclass=None, rawid=None):
    ...
    for i, cid in enumerate(cellids):
        ...
        respfile= spike_cache_filename2(d['raster'],parms)
        for j, rf in enumerate(respfile):
            rts = load_matlab_matrix(rf,key="r")
...
def load_pup_raster(batch, site, options,runclass=None,rawid=None):
    ...
    pupfile=pupil_cache_filename2(files,options)
    a_p = []
    for j, rf in enumerate(pupfile):
        
        pts = load_matlab_matrix(pupfile.iloc[j],key='r')
...
def baphy_stim_cachefile(exptparams, options, parmfilepath=None):
    ...
    if 'OveralldB' in exptparams['TrialObject'][1]:
        ...
        OveralldB=0
    # print(options)
    dstr=dstr+"-{0}-fs{1}-ch{2}".format(options['stimfmt'],
                                        options['rasterfs'],
                                        options['chancount'])
...
def baphy_load_recording(cellid, batch, options):
    ...
    options = {**default_options, **options}  # Merge, preferring 'options'
